...
class FullyAssocLiFoCache implements Cache {
    ...
    private int[] cache_storage;    //1D int array
    
    // - One to manage locations in the cache
    private Stack<Integer> location_stack;    //Stack of Integers
    
    // And because the cache is Fully Associative:
    // - One to translate between memory addresses and cache locations
    private HashMap<Integer, Integer> address_to_cache_loc; //Hashmap mapping address to cache location
    // - One to translate between cache locations and memory addresses  
    private HashMap<Integer, Integer> cache_loc_to_address; //vice versa
    ...
    private boolean cache_is_full(){
         // Your code here
         //Cache is full when there are no entries in location stack
    	//as location stack holds free locations
